...
// #6.1 매개변수 포맷 검증 : 검증을 위해서 zod 사용(URL을 parse하는 schema)
...
/*
  meta 함수는 탭 꾸며주기 위해서 사용
  params 매개변수를 받아옴
  root에서 한 설정이 meta 태그에는 적용되지 않을 것 같아서 setZone("Asia/Seoul").setLocale("ko") 설정을 해줌

  meta 함수에서 parameter와 loader 함수에서 제공하는 data도 받을 수 있음
...
export const loader = async ({ params, request }: Route.LoaderArgs) => {
  ...
  */
  ...
  // 작성날짜가 미래인지 확인
  const today = DateTime.now().setZone("Asia/Seoul").startOf("day"); // 00:00:00 시간
  if (date > today) {
    throw data(
      {
